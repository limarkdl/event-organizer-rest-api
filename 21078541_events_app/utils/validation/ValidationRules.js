...
export const userValidationSchema = Joi.object({
  username: Joi.string().alphanum().min(1).required().messages({
...
export const eventValidationSchema = Joi.object({
  ...
  price: Joi.number().min(1).max(Number.MAX_SAFE_INTEGER).required().messages({
  ...
  numOfParticipants: Joi.number().equal(0),
});

export const reservationValidationSchema = Joi.object({
  eventID: Joi.number().min(1).max(Number.MAX_SAFE_INTEGER).required(),
  userID: Joi.number().min(1).max(Number.MAX_SAFE_INTEGER).required(),
});
