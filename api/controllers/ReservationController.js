...
class ReservationController {
    ...
    static async createReservation(req, res) {
        ...
        try {
            ...
            const {error} = ValidationRules.reservationValidationSchema.validate(creationData, {abortEarly: false})
            ...
            if (existingReservation) {
                await transaction.rollback()
                return res.status(422).send({message: 'User already has a reservation for this event.'})
            }

            const reservationCount = await db.Reservation.count({where: {eventID}, transaction})

            if (reservationCount >= event.maxParticipants) {
                await transaction.rollback()
                return res.status(422).send({message: 'No available slots for this event.'})
            }

            const newReservation = await db.Reservation.create({eventID, userID}, {transaction})
