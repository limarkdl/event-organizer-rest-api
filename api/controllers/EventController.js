...
class EventController {
    static async getAllEvents(req, res) {
        try {
            ...
            if (dateTime) {
                ...
                conditions.dateTime = {[db.Sequelize.Op.gte]: timestamp}
            }
            ...
            if (userIDs) {
                ...
                includeUsers = [{
                    ...
                    where: {userID: {[db.Sequelize.Op.in]: userIdList}},
    ...
    static async createEvent(req, res) {
        ...
        try {
            ...
            const {error} = ValidationRules.eventValidationSchema.validate(creationData, {abortEarly: false})
            ...
            const eventType = await db.EventType.findByPk(eventTypeID, {transaction})
            if (!eventType) {
                await transaction.rollback()
                return res.status(422).send({message: 'eventTypeID was not found.'})
            }

            const organizer = await db.Organizer.findByPk(organizerID, {transaction})
            if (!organizer) {
                await transaction.rollback()
    ...
    static async updateEvent(req, res) {
        ...
        try {
            ...
            const {error} = ValidationRules.eventValidationSchema.validate(updateData, {abortEarly: false})
            ...
            if (updateData.eventTypeID) {
                const eventType = await db.EventType.findByPk(updateData.eventTypeID, {transaction})
                if (!eventType) {
                    await transaction.rollback()
                    return res.status(422).send({message: 'eventTypeID was not found.'})
                }
            ...
            if (updateData.organizerID) {
                const organizer = await db.Organizer.findByPk(updateData.organizerID, {transaction})
                if (!organizer) {
                    await transaction.rollback()
                    return res.status(422).send({message: 'organizerID was not found.'})
                }
    ...
    static async deleteEvent(req, res) {
        ...
        try {
            ...
            if (reservationCount > 0) {
                await transaction.rollback()
                return res.status(422).send({message: 'Event cannot be deleted because it is referenced by reservations.'})
            }
